...
<section class="relative min-h-screen mb-20 text-center space-y-10 border-2 border-main-white rounded-md p-2">
    ...
    <div class="absolute top-0 w-full flex justify-around">
        <button on:click={() => navigate('prev')}><Icon class="w-10 h-10 glow-fx hover:text-gold transition-colors" icon="icon-park-outline:left-c" /></button>
        <button on:click={() => navigate('next')}><Icon class="w-10 h-10 glow-fx hover:text-gold transition-colors" icon="icon-park-outline:right-c" /></button>
    </div>
